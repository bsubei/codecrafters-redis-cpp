...
TEST(MessageTest, MessageToString)
{
    ...
    EXPECT_EQ(message_to_string(Message("ECHO", DataType::BulkString)),
              "$4\r\nECHO\r\n");
    EXPECT_EQ(message_to_string(Message(std::vector<Message>{}, DataType::Array)),
              "*0\r\n");
    EXPECT_EQ(
        message_to_string(Message(std::vector<Message>{
...
TEST(MessageTest, MessageFromString)
{
    ...
    EXPECT_EQ(message_from_string("*0\r\n"), Message(std::vector<Message>{}, DataType::Array));
    EXPECT_EQ(
...
TEST(MessageTest, RoundTrip)
{
    ...
    const auto m3 = Message(std::vector<Message>{m1, m2}, DataType::Array);
    const auto empty = Message(std::vector<Message>{}, DataType::Array);
