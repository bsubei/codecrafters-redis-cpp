...
std::ostream &operator<<(std::ostream &os, const Message &message) {
  ...
  std::visit(
      MessageDataVisitor{
          [&os](const Message::NestedVariantT &message_data) {
            os << "vector of size " << message_data.size() << ": [\n";
            for (const auto &m : message_data) {
              assert(std::holds_alternative<Message::StringVariantT>(m.data) &&
          ...
          [](const auto &&message_data) {
            std::cerr << "Unable to call operator<< on message with data: "
